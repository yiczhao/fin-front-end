...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = {
	    ...
	    methods: {
	        ...
	        getInfo: function getInfo(a, index) {
	            var _this = this;

	            //                this.zdlists[index].listinfo = []
	            if (this.listinfos[index] != '' && typeof this.listinfos[index] != 'undefined') return;
	            this.$http.post('./reservecash/order/getpart/' + a.id).then(function (response) {

	                // *** 判断请求是否成功如若成功则填充数据到模型
	                //  this.zdlists.$set( index,info)
	                response.data.code == 0 ? _this.listinfos.$set(index, response.data.data) : null;
	                console.log((0, _stringify2.default)(_this.listinfos));
	            }, function (response) {
	                console.log(response);
	            });
	        },
