...
export default function install(Vue,router_proto) {
	...
	Vue.http.interceptors.push({
		...
		response (response) {
			...
			if(request.url.indexOf('auth/usersystem/list')<=0&&response.request.url.indexOf('subCompany/list')<=0&&response.request.url.indexOf('city/list')<=0&&response.request.url.indexOf('/total')<=0&&response.request.url.indexOf('/sum')<=0){
				Message.hide();
