...
;(function (window, KSAuthKit) {
    ...
    function setCookie (key, value, expire, domain) {
        ...
        document.cookie = key + '=' + escape(value) +
            ((expire === null) ? '' : ';expires=' + data.toGMTString()) + ';domain=' + domain;
    }
    ...
    function getAuthMapper (cb, errCb) {
        ...
        // 缓存中不存在 则 从远端读取
        // 拼装请求数据
        var requestData = '?systemId=' + _config.systemId +
            '&userToken=' + encodeURIComponent(_config.userToken)
        // var requestData = {
        //   sys_id: _config.systemId,
        //   user_token: encodeURIComponent(_config.userToken)
        // }
        // 发送请求
        xhrReq({
            method: 'GET',
            address: _config.apiURL + requestData
        }, function (data) {
            var authM, authSys

            // 远端数据返回 检查数据是否正确 并存入缓存
            authM = (data.data && data.data.permission) || []
            authSys = (data.data && data.data.system_infos) || []
            _SS.setItem('KSAuthMapper', JSON.stringify(authM))
            _SS.setItem('KSAuthSysList', JSON.stringify(authSys))
    ...
    _o.destroy = function () {
        // 移除用户 cookie
        setCookie('KSAuthUserToken', '', -1, '.kashuo.net')
        setCookie('KSAuthJSURL', '', -1, '.kashuo.net')
        setCookie('KSAuthApiURL', '', -1, '.kashuo.net')
        setCookie('KSAuthUserName', '', -1, '.kashuo.net')
    ...
    _o.watchDog = function () {
        ...
        // trim 掉多余的空格
        AuthMapper = AuthMapper.map(function (o) {
            return o.trim()
        })
        ...
        for (var j = 0; j < controlNodeLength; j++) {
            ...
            var operation = node.getAttribute('data-ksa').trim()
